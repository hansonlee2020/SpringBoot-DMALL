...
public class GlobalExceptionHandlerAspect {
    ...
    private final static Logger logger = LoggerFactory.getLogger(GlobalExceptionHandlerAspect.class);

    @Pointcut("execution(* com.hanson.service.impl.*.*(..))")
    public void exceptionPointcut(){}

    @Pointcut("execution(* com.hanson..*.*(..))")
...
//    }
    ...
    @AfterThrowing(pointcut = "globalPointcut()",throwing = "exception")
    public void globalExceptionRecord(JoinPoint joinPoint,Throwable exception){
        LogMessage logMessage = new LogMessage();
        logMessage.setTarget(joinPoint.getSignature().toString());
        logMessage.setArgs(Arrays.toString(joinPoint.getArgs()));
        logMessage.setKind(joinPoint.getKind());
